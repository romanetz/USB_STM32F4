...
extern s16 Audio[384];
extern uint8_t * xbuf;
extern s16 MPX_buf[4608] __attribute__ ((aligned(4)));
extern uint32_t fs;
extern uint8_t mult_factor;
...
static uint8_t  AudioCmd(uint8_t* pbuf, 
                         ...
                         uint8_t cmd)
 {
  ...
  case AUDIO_CMD_PLAY:
    ...
    {
      memcpy(Audio,pbuf,size);
      audio_buffer_fill=0;
      Audio_MAL_Play(&Audio[0],size/2);// (size/4));
      AudioState = AUDIO_STATE_PLAYING;
      return AUDIO_OK;
    }
