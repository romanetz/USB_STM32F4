...
{  
 ...
 #error "Missing define: Evaluation board (ie. USE_STM322xG_EVAL)"
  ...
  LCD_LOG_SetHeader(" USB OTG HS HID/MSC Device");
#else
  LCD_LOG_SetHeader(" USB OTG FS HID/MSC Device");
#endif
  LCD_UsrLog("> USB device library started.\n"); 
  LCD_LOG_SetFooter ("     USB Device Library v2.1.0" ); 
...
void USBD_USR_DeviceReset(uint8_t speed )
{
 switch (speed)
 {
   case USB_OTG_SPEED_HIGH: 
     LCD_LOG_SetFooter ("     USB Device Library v2.1.0 [HS]" );
     break;

  case USB_OTG_SPEED_FULL: 
     LCD_LOG_SetFooter ("     USB Device Library v2.1.0 [FS]" );
     break;
 default:
     LCD_LOG_SetFooter ("     USB Device Library v2.1.0 [??]" );
...
{
  LCD_UsrLog("> HID/MSC Interface started.\n");
}
