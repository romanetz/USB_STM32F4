...
void SysTick_Handler(void)
{
  ...
  {
    buf = USBD_HID_GetPos();//заполняется буфер
    //эта функция не связана с драйвером usb
    if((buf[1] != 0) ||(buf[2] != 0))
    {
    ...
    if (Counter == 10)
    {      
      if (STM_EVAL_PBGetState(BUTTON_USER) == Bit_SET) {Buffer[0] = 100;STM_EVAL_LEDOn(LED6);} else {Buffer[0] = 0;STM_EVAL_LEDOff(LED6);}
      Counter = 0x00;
    }  
  }
