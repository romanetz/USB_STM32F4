...
volatile uint8_t mpx_buff_fill;
/*----------------------------------------------------------------------------*/
...
void SetI2SMCLK(uint32_t freq)
{
uint32_t I2SFREQ[]={8000,16000,32000,48000,22050,44100};
uint32_t PLLI2SN[]={256,426,426,258,344,429,271};
uint32_t PLLI2SR[]={5,2,2,3,2,2,2};
/*
uint32_t I2SDIV[]={6,13,3,3,3,9,3};
uint32_t I2SODD[]={0,1,1,1,1,0};
*/
uint32_t I2SDIV[]={6,13,3,4,3,9,3};
uint32_t I2SODD[]={0,1,1,0,1,0};

uint8_t index=0;
while ((freq!=I2SFREQ[index])&&index<7) index++;
if (index==6) return;
/******************************************************************************/
...
CODEC_I2S->I2SPR = (uint16_t)((uint16_t)I2SDIV[index] | (uint16_t)((I2SODD[index]<<8)));// | (uint16_t)I2S_MCLKOutput_Enable));
CODEC_I2S->I2SCFGR|=SPI_I2SCFGR_I2SE;
}
...
{ 
  ...
  //DMA_ClearFlag(AUDIO_MAL_DMA_STREAM, AUDIO_MAL_DMA_FLAG_TC|AUDIO_MAL_DMA_FLAG_HT);//при отключенных прерываниях dma без этого ошибка
...
#ifndef DAC_USE_I2S_DMA //if not defined DAC USE I2S DMA - what triggers DAC DMA?...
  else
  {
    dac_play_cnt = 0;
    dac_play_ptr = (uint16_t*)Addr;
...
void Audio_DAC_play(uint8_t* Addr, uint32_t Size)
{
	/* Enable the I2S DMA Stream*/
    DMA_Cmd(AUDIO_DAC_DMA_STREAM, DISABLE);
	/* Configure the buffer address and size */
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)Addr;
    DMA_InitStructure.DMA_BufferSize = (uint32_t)Size;

    /* Configure the DMA Stream with the new parameters */
    DMA_Init(AUDIO_DAC_DMA_STREAM, &DMA_InitStructure);

    /* Enable the I2S DMA Stream*/
    DMA_Cmd(AUDIO_DAC_DMA_STREAM, ENABLE);
    /* Enable DAC Channel2 */
    DAC_Cmd(AUDIO_DAC_CHANNEL, ENABLE);
...
uint8_t dds_spi_buf[8]={0,0,0,4,0x40,0x40,0x40,0x40};

void DDS_Play(uint8_t* Addr, uint32_t Size)
{
...
void DAC_Config(void)
{
  ...
  DAC_Init(AUDIO_DAC_CHANNEL, &DAC_InitStructure);
  	...
  	/* Enable the DMA clock */
  	    ...
  	    /* Configure the DMA Stream */
  	    DMA_Cmd(AUDIO_DAC_DMA_STREAM, DISABLE);
  	    DMA_DeInit(AUDIO_DAC_DMA_STREAM);
  	    /* Set the parameters to be configured */
  	    DMA_InitStructure.DMA_Channel = AUDIO_DAC_DMA_CHANNEL;
  	    DMA_InitStructure.DMA_PeripheralBaseAddr = AUDIO_DAC_DMA_DREG;
  	    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)0;      /* This field will be configured in play function */
  	    DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
  	    DMA_InitStructure.DMA_BufferSize = (uint32_t)0xFFFE;      /* This field will be configured in play function */
  	    DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
  	    ...
  	    DMA_ITConfig(AUDIO_DAC_DMA_STREAM, DMA_IT_TC|DMA_IT_HT, ENABLE);

  	    /* DAC DMA IRQ Channel configuration */
  	    NVIC_InitStructure.NVIC_IRQChannel = AUDIO_DAC_DMA_IRQ;
  	    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = EVAL_AUDIO_IRQ_PREPRIO;
  	    NVIC_InitStructure.NVIC_IRQChannelSubPriority = EVAL_AUDIO_IRQ_SUBRIO;
  	    NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  	    NVIC_Init(&NVIC_InitStructure);
...
void TMR6_Config(uint32_t freq, uint32_t SYSFREQ)
{
	...
	/* Time base configuration */
  	...
  	TIM_TimeBaseStructure.TIM_Prescaler         = (SYSFREQ/freq)-1;
...
void TMR2_Config(uint32_t freq, uint32_t SYSFREQ)
{
	...
	TIM_OCInitStructure.TIM_Pulse = 0x45A1CAC0; // I/O update pulse length 3 periods
    ...
    //RCC_AHB1PeriphClockCmd(AUDIO_DAC_DMA_CLOCK, ENABLE);
  	...
  	/* Set the parameters to be configured */
  	/*DMA_InitStructure.DMA_Channel = AUDIO_DAC_DMA_CHANNEL;
  	DMA_InitStructure.DMA_PeripheralBaseAddr = TIM2.DR;
  	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)0;      /* This field will be configured in play function */
  	/*DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
  	DMA_InitStructure.DMA_BufferSize = 1;      /* This field will be configured in play function */
  	/*DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
  	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
...
void TMR2_Config(uint32_t freq, uint32_t SYSFREQ)
{
	...
	/* Time base configuration */
  	TIM_TimeBaseStructure.TIM_Period            = 0xffffffff;
//  	freq_exact=SYSFREQ*1.0/TIM_TimeBaseStructure.TIM_Period;
...
#endif
//PB7=12288 kHz input
//PB8=384 kHz PWM output, duty cycle = 5/32 (DDS I/O update)

void TMR4_Config(uint8_t * buf, uint32_t len)
{
	  TIM_OCInitTypeDef TIM_OCInitStructure;
	  TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
	  GPIO_InitTypeDef GPIO_InitStructure;
	  ...
	  // TIM4_CH2 (PB.07) and TIM4_CH3 pin (PB.08) configuration
	  ...
	  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1;
  	  ...
  	  TIM_OCInitStructure.TIM_Pulse = 29; // I/O update pulse length 31-19 periods of master clock
  	  TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;
	  ...
	  //DAC is updated with OC3REF trigger, with DDS
	  //this signal also runs mem2mem DMA FTW buffer -> DDS buffer (5 bytes)
	  ...
	  /* Configure the DMA Stream for FTW_buf to TIM2->CCR2 transmission*/
	  /* TIM2 CCR2 is used as buffer for frequency component of FTW, not as regular use!*/
	  DMA_Cmd(DMA1_Stream0, DISABLE);
	  DMA_DeInit(DMA1_Stream0);
	  /* Set the parameters to be configured */
	  DMA_InitStructure.DMA_Channel = DMA_Channel_2;
	  DMA_InitStructure.DMA_PeripheralBaseAddr = (TIM2_BASE+0x40);
	  DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)buf;      /* This field will be configured in play function */
	  DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
	  DMA_InitStructure.DMA_BufferSize = (uint32_t)len;      /* This field will be configured in play function */
	  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
	  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
	  DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word;
...
void TMR3_Config(void)
{
      ...
      // TIM3_ETR (PD.02) and TIM3_CH1 pin (PB.04) configuration
	  ...
	  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1;
  	  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  	  TIM_OCInitStructure.TIM_Pulse = 1; // I/O update pulse length 3 periods
  	  TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;
	  ...
	  //DAC is updated with OC3REF trigger, with DDS
	  //this signal runs mem2periph DMA DDS buffer -> SPI (1 bytes 5 times per TIM4 cycle)
	  ...
	  /* This DMA moves 5 bytes from the middle buffer to SPI2, 1 byte per transaction
	   * as the DDS needs 5 bytes in the format of:
	   * 0x04 FTW[7:0] FTW[15:8] FTW[23:16] FTW[31:24] - for AD9951-like
	   * FTW[7:0] FTW[15:8] FTW[23:16] FTW[31:24] 0x00 - for AD9850-like
	   * */
	  DMA_InitStructure.DMA_Channel = DMA_Channel_5;
	  DMA_InitStructure.DMA_PeripheralBaseAddr = (SPI2_BASE+0x0C);
	  DMA_InitStructure.DMA_Memory0BaseAddr =  (uint32_t)&dds_spi_buf[3];      /* This field will be configured in play function */
	  DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
	  DMA_InitStructure.DMA_BufferSize = 5;      /* This field will be configured in play function */
	  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
	  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
...
//	  DMA_Init(AUDIO_DAC_DMA_STREAM, &DMA_InitStructure);
   ...
   /* This DMA moves 4 bytes from the TMR2.CCR2 to middle buffer, 1 word per transaction
   * Depending on the DDS chip, DMA transfers FTW before (AD9850-like) constant parameter
   * or after it (AD9951-like).
   *
   * as the DDS needs 5 bytes in the format of:
   * 0x04 FTW[7:0] FTW[15:8] FTW[23:16] FTW[31:24] - for AD9951-like
   * FTW[7:0] FTW[15:8] FTW[23:16] FTW[31:24] 0x00 - for AD9850-like
   * */
  	  DMA_InitStructure.DMA_Channel = DMA_Channel_5;
  	  DMA_InitStructure.DMA_PeripheralBaseAddr = (TIM2_BASE+0x40);
  	  DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)&dds_spi_buf[4];      /* This field will be configured in play function */
  	  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
  	  DMA_InitStructure.DMA_BufferSize = 0x1;      /* This field will be configured in play function */
  	  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
...
//TIM3 and its DMA transactions generates no interrupts
/* Enable the selected DMA interrupts (selected in "stm32f4_discovery_eval_audio_codec.h" defines) */
...
/**
  ...
  * Эта функция вызывается с удвоенной частотой семплирования (48кгц -> 96 кгц)
  */
//Это ж ужас - прерывания с частотой 96 кГц!!!!!!
